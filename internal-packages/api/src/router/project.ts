...
export const projectRouter = {
  ...
  getProject: projectProcedure.query(({ ctx }) => {
    return {
      ...
      limits: {
        concurrent: ctx.project.concurrentConnectionsLimit,
        incomingMessages: ctx.project.incomingMessagesLimit,
        outgoingMessages: ctx.project.outgoingMessagesLimit,
      },
    };
  }),
  ...
  createProject: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const project = await ctx.db.transaction(async (tx) => {
        ...
        const project = await createProject(ctx.db, input.name, ctx.user.id);
