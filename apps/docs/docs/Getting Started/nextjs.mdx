...
1. ### Installing packages
   ...
   <Tabs groupId="package-manager">
      ...
      ```bash
      npm install socketless.ws
      ```
      </TabItem>
      <TabItem value="pnpm" label="pnpm">
      ```bash
      pnpm add socketless.ws
      ```
      </TabItem>
      <TabItem value="bun" label="bun">
      ```bash
      bun add socketless.ws
      ```
      </TabItem>
      <TabItem value="yarn" label="yarn">
      ```bash
      yarn add socketless.ws
      ```
      </TabItem>
   </Tabs>
...
2. ### Setting environment variables
   ...
   You can get the `SOCKETLESS_CLIENT_ID` and `SOCKETLESS_TOKEN` from the [socketless.ws dashboard](https://socketless.ws/dashboard).
...
Check out how to develop locally with Socketless on Next.js.
